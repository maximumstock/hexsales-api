...
module.exports = {
     ...
     * @param {Object} articleHashMap - A hashmap of all articles mapped by their sanitized names (eg. 'Argus Herald of Doom' => {name: 'Argus, Herald of Doom', rarity: 'Legendary', ...})
    ...
    parseSales: function(body, articleHashMap) {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            const mappedArticle = articleHashMap[internalName.toLowerCase()];

            if(!mappedArticle) {
                logger.info(`Could not find any matching articles in hexdb for ${internalName}`);
                continue;
            }
        ...
        // sort array by date, currency, uuid
        const sortedSales = _.sortBy(sales, ['date', 'currency', 'uuid']);
