...
module.exports = {
  ...
  parseSales: function(body) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      // establish default data on application level, as knex defaults do not work sometimes(?!)
      // at least it didn't work with `aa` and `ea`
      sale.aa = false;

      // sale related data
      sale.name = content[0].trim();
      sale.rarity = parseInt(content[1].trim());
      sale.currency = content[2].trim().toLowerCase() === 'platinum' ? 1 : 2;
      sale.price = parseInt(content[3].trim());
      sale.date = content[4].trim();

      // determine more properties
      if (sale.rarity === 5) {
        sale.aa = true; // AA version of a card
      }
