    ...
    objectNames: function() {
        return new Promise(function(resolve, reject) {
            ...
            request(options, function(error, response, body) {
                ...
                // gather all names
                const uniqueNames = _.uniq(body.map(function(i) {
                    return i.name;
                }));

                // transform `uniqueObjectNames` so it can be used more easily
                const minifiedUniqueObjectNames = uniqueNames.map(function(n) {
                    let ret = n;
                    // split name by all special characters HexENT cannot deal with
                    [','].forEach(function(specialCharacter) {
                        ret = ret.split(specialCharacter).join('');
                    });
                    return {
                        ename: n,
                        name: ret
                ...
                // build a hashmap to we have contant time access
                let minifiedUniqueObjectMap = {};
                minifiedUniqueObjectNames.forEach(function(e) {
                    let key = e.name.split(' ').join('').toLowerCase();
                    minifiedUniqueObjectMap[key] = e.ename;
                });

                return resolve(minifiedUniqueObjectMap);
