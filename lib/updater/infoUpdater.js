...
module.exports = {
  ...
  updateSingle: function(article, names, callback) {
    ...
    // check if `names` contains the `internal` string of the article
    const foundNames = names.filter(function(n) {
      return n === article.internal;
    });
    ...
    request(options, function(error, response, body) {
      ...
      if (body.length === 0) {
        logger.info(`there is no data for ${article.internal}, ${article.rarity}`);
        return callback(null, null);
      }
      ...
      if(article.type === 'Equipment') {
        updatedArticle.name = newData.name;
        updatedArticle.rarity = newData.rarity;
        updatedArticle.slot = newData.sub_type;
