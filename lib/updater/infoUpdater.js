...
module.exports = {
  ...
  updateSingle: function(article, names, callback) {
    ...
    request(options, function(error, response, body) {
      ...
      if(body.length > 1) {
        logger.info(`too much hexdatabaseapi data for ${article.internal}, ${article.rarity} (${JSON.stringify(body)})`);
      }
      ...
      // copy data from the reponse so we don't have to know its contents
      let newData = body.filter(function(u) {
        return u.name.length === mappedName.length;
      })[0];
      ...
      if(article.type === 'Card') {
        ...
        if(article.aa === true) {
          updatedArticle.aa = true; // explicity evaluate to true, so false values are not copied
        }
        // copy `ea` if it is true
        if(article.ea === true) {
          updatedArticle.ea = true; // explicitly evaluate to true
        }
