        ...
        return new Promise(function(resolve, reject) {
            ...
            dbm.getArticleHashMap()
                ...
                .then(function(updatesDone) {
                    ...
                    const today = moment();
                    const lastUpdate = moment(updatesDone[updatesDone.length - 1]);

                    const differenceInDays = today.diff(lastUpdate, 'days');
                    const updatesToDo = []
                    for(let i = 0; i < differenceInDays; i++) {
                      updatesToDo.push(lastUpdate.add(1, 'days').format('YYYY-MM-DD'));
                    }
                    ...
                    if(config.startDate) {
                        logger.info(`Only inserting data beginning from ${config.startDate}`);
                    }
