...
module.exports = {
        ...
        return new Promise(function(resolve, reject) {
            ...
            dbm.getUpdatesAvailable(config.cndIndex)
                .then(function(_updatesAvailable) {
                    ...
                    return dbmanager.objectNames();
                })
                .then(function(_objectNames) {
                    uniqueObjectNames = _objectNames;
                ...
                .then(function(updatesDone) {
                    ...
                    updatesToDo.forEach(function(date) {
                        funcArray.push(function(_callback) {
                            module.exports.startSingle(date, uniqueObjectNames, _callback);
                        });
                    });
     ...
     */
    startSingle: function(date, uniqueObjectNames, callback) {
        ...
        request.get(url, function(error, response, body) {
            ...
            const sales = utils.parseSales(body, uniqueObjectNames);
...
function insertDailySales(sales, cb) {
    ...
    for (let i = 0; i < sales.length; i++) {
        ...
        if (i === sales.length - 1 || (oldSale !== null && (s.date !== oldSale.date || s.currency !== oldSale.currency || s.name !== oldSale.name ||  s.rarity !== oldSale.rarity))) {
