...
class Summary {
    ...
    static * get(params) {
        ...
        const query = [
            'SELECT currency as c, sum(total)::int as total, sum(quantity)::int as quantity, (sum(total)/sum(quantity))::int as avg, min(min)::int as min, max(max)::int as max',
            'FROM daily_sales',
            'WHERE date >= ? and date <= ?',
            'AND name LIKE ?',
            'AND uuid LIKE ?',
     ...
     * @param {String} name - The name of the article to get a summary for
     * @param {Object} params - A parameter object
    ...
    static * getForArticle(name, params) {
        ...
        const query = [
            'SELECT currency as c, sum(price) as total, count(1) as quantity, min(price)::int as min, max(price)::int as max, avg(price)::int as avg, median(price)::int as median',
            'FROM sales',
            'WHERE date >= ? and date <= ?',
            'AND name = ?',
            'GROUP BY currency'
