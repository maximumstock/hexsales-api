...
class Stats {
    ...
    static * getMostSoldArticles(_start, _end, _limit) {
        ...
        for (let i = 0; i < currencies.length; i++) {
            ...
            const query = [
                'SELECT uuid, name, rarity, set, type, sum(quantity)::int as quantity, sum(total)::int as total, (sum(total)/sum(quantity))::int as average',
                'FROM daily_sales',
                'WHERE date >= ? and date <= ? and currency = ?',
                'GROUP BY uuid, name, rarity, set, type',
